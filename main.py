...
green = Fore.GREEN
reset = Fore.RESET
screen = f'''Made by @harry.sfo / brivexvs
t.me/hcaptchamodels
t.me/hcaptchamodels
t.me/hcaptchamodels
...
def title():
    ctypes.windll.kernel32.SetConsoleTitleW("Tokens: %s | Total Requests: %s | Authorized: %s | Failed: %s" % (
    ...
    for _ in range(max_retries):
        ...
        try:
            ...
            if req_one.status_code in (200, 201, 204):
                ...
                value = urlsafe_b64decode(token.split(".")[
                                                      0]+"==").decode("utf-8")
                if include_tokens:
                    authed_token = f"{value}:{access_token}:{refresh_token}:{token}\n"
                else:
                    authed_token = f"{value}:{access_token}:{refresh_token}\n"
